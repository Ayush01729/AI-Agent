...
@app.post("/chat/stream/v1")
async def chat_stream_v1(request: ChatRequest):
    """
    Streaming endpoint V1 - Testing Gemini 2.5 Flash streaming support.
    ...
    try:
        ...
        async def event_generator():
            ...
            try:
                ...
                logger.error(f"Error in streaming v1: {str(e)}")
                error_msg = json.dumps({"error": str(e)})
                yield f"data: {error_msg}\n\n"
        ...
        return StreamingResponse(
            ...
            headers={
                ...
                "X-Accel-Buffering": "no",
                "Content-Type": "text/event-stream; charset=utf-8",
                "X-API-Version": "v1-gemini"  # Version indicator
            }
        )
    except Exception as e:
